...
def plot_edge():
    ...
    # Get vertical edge
    vertical_edge_nodes = get_vertical_edge(G, pos)
...
boundary_pos = {node: pos[node] for node in vertical_edge_nodes}
q_boundary= list(boundary_pos.values())
r1_boundary=[]
for i in np.arange(0,len(boundary_pos)):
    s=q_boundary[i][0]
    r1_boundary.append(s)
r2_boundary=[]
for i in np.arange(0,len(boundary_pos)):
    s=q_boundary[i][1]
    r2_boundary.append(s)

A_boundary=np.zeros((len(boundary_pos),3))
for i in np.arange(0, len(boundary_pos)):
    A_boundary[i][1]= r1_boundary[i]
    A_boundary[i][2]= r2_boundary[i]
    A_boundary[i] =i
for i in np.arange(0, len(boundary_pos)):
    A_boundary[i][1]= r1_boundary[i]
    A_boundary[i][2]= r2_boundary[i]
...
vertical_edge_indices = [coord_to_index[tuple(pos[node])] for node in vertical_edge_nodes]

print("Indices of vertical edge nodes in A:", vertical_edge_indices)
...
for t2 in t2_values:
    ...
    @njit
    def calculate_current(solutions, t_index, time_steps, Ht, A_boundary, vertical_edge_indices):
    ...
    J_t = [calculate_current(solutions, t_index, time_steps, Ht, A_boundary, vertical_edge_indices) for t_index in range(len(time_steps))]
...
plt.grid(True)
plt.savefig(f'plots/edge_currents_amplitude_{num}.pdf')
plt.show()
